...
float perlin_noise( vec3 st) 
{
  ...
  //first, get grid of random gradient vectors
  ...
  float fx1 = n1 + smooth_step(st.x - float(x0)) * (n2 - n1);
  float fx2 = n3 + smooth_step(st.x - float(x0)) * (n4 - n3);
  float fx3 = n5 + smooth_step(st.x - float(x0)) * (n6 - n5);
  float fx4 = n7 + smooth_step(st.x - float(x0)) * (n8 - n7);
  float fy1 = fx1 + smooth_step(st.y - float(y0)) * (fx2 - fx1);
  float fy2 = fx3 + smooth_step(st.y - float(y0)) * (fx4 - fx3);
  float fz = fy1 + smooth_step(st.z - float(z0)) * (fy2 - fy1);
  return fz/3; //smooth value between -1 and 1
  /////////////////////////////////////////////////////////////////////////////
}
